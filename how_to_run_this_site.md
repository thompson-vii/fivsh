...
image_preproc.py is used to generate image metadata as photoswipe needs img dimension to work. The generated json is placed under `/_data` and named same as the md template. 

beside the extension the two files need to have the same filename. The variable inside the json is then made avaliable to the md template for use.
