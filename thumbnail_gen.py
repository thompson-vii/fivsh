...
def write_fullres_to_thumb(fullres_path: Path, thumbnail_path: Path, thumbnail_size: list[str], dry_run=False) -> None:
...
def thumbnail_gen(dry_run: bool):
    ...
    global EXT
    EXT = ['.jpg', '.png', '.jpeg'] # period for x.suffix
    ...
    if missing_pieces:
        for img in missing_pieces:
            ...
            write_fullres_to_thumb(og_img_path, thumbnail_path, THUMBNAIL_MAX_SIZE, dry_run)
    else:
